          ...
          <div className={`text-xs sm:text-sm leading-relaxed wrap-break-word ${isUser ? "text-white" : "text-gray-800"}`}>
            <ReactMarkdown 
              ...
              components={{
                ...
                h1: ({ children }) => <h1 className={`text-xl font-bold mt-4 mb-2 ${isUser ? "text-white" : "text-gray-900"}`}>{children}</h1>,
                h2: ({ children }) => <h2 className={`text-lg font-bold mt-3 mb-2 ${isUser ? "text-white" : "text-gray-900"}`}>{children}</h2>,
                h3: ({ children }) => <h3 className={`text-base font-bold mt-2 mb-1 ${isUser ? "text-white" : "text-gray-900"}`}>{children}</h3>,
                p: ({ children }) => <p className="my-2">{children}</p>,
                strong: ({ children }) => <strong className="font-bold">{children}</strong>,
                em: ({ children }) => <em className="italic">{children}</em>,
                a: ({ href, children }) => <a href={href} className={`underline ${isUser ? "text-green-200" : "text-green-600"} hover:opacity-80`} target="_blank" rel="noopener noreferrer">{children}</a>,
                ul: ({ children }) => <ul className="list-disc pl-5 my-2 space-y-1">{children}</ul>,
                ol: ({ children }) => <ol className="list-decimal pl-5 my-2 space-y-1">{children}</ol>,
                code: ({ inline, children }: { inline?: boolean; children?: React.ReactNode }) => 
                  inline 
                    ? <code className="bg-gray-200 text-gray-800 px-1 py-0.5 rounded text-xs break-all">{children}</code>
                    : <code className="block bg-gray-800 text-white p-2 sm:p-3 rounded my-2 overflow-x-auto text-xs">{children}</code>,
                blockquote: ({ children }) => <blockquote className={`border-l-4 pl-4 my-2 italic ${isUser ? "border-green-300" : "border-gray-300"}`}>{children}</blockquote>,
              }}
            >
              {message.content}
            </ReactMarkdown>
