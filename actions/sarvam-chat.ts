...
function getFriendlySarvamError(error: unknown): string {
  const message =
    error instanceof Error ? error.message : "Failed to get chat response";

  if (message.includes("Sarvam API key not configured")) {
    return "Sarvam API key is missing. Add SARVAM_API_KEY to .env.local and restart the server.";
  }

  if (
    message.includes("Sarvam API error: 401") ||
    message.includes("Sarvam API error: 403") ||
    message.includes("invalid_api_key_error")
  ) {
    return "Sarvam API authentication failed. Check SARVAM_API_KEY in .env.local and restart the server.";
  }

  return message;
}

function getSarvamHeaders(apiKey: string): Record<string, string> {
  ...
  if (!trimmedKey.toLowerCase().startsWith("bearer ")) {
    headers["api-subscription-key"] = trimmedKey;
  }
...
6. Keep answers clear, concise, and easy to understand for farmers with varying education levels${contextInfo}`;
    ...
    const response = await fetch(
      ...
      {
        method: "POST",
        headers: getSarvamHeaders(apiKey),
    ...
    return {
      success: false,
      error: getFriendlySarvamError(error),
    };
  }
}
