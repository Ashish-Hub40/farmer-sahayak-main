  ...
  useEffect(() => {
    if (!messagesLoaded) {
      loadSessionMessages(sessionId).then((result) => {
        ...
        console.warn("Database not available, chat will work in memory only:", error);
        setMessagesLoaded(true);
      });
    }
  ...
  const sendMessage = useCallback(
    async (text: string, imageBase64?: string) => {
      ...
      try {
        ...
        // Build context
        const context: { weather?: string; plantHealth?: string } = {};
        ...
        // Get AI response from server-side proxy route
        ...
        if (!response.ok || !chatResult.success || !chatResult.data) {
